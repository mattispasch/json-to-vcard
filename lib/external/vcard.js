...
// based on https://github.com/mattt/vcard.js, added "exports.vCard"
...
exports.vCard = vCard = {
  ...
  parse: function(_input, fields) {
    ...
    assert(lines[0] == "BEGIN:VCARD", "vCard must start with 'BEGIN:VCARD', but was: " + lines[0]);
    assert(lines[lines.length -1] == "END:VCARD", "vCard must end with 'END:VCARD', but was: " + lines[lines.length-1]);
    for (n in lines) {
      line = lines[n];
      ...
      if(regexps['simple'].test(line))
      {
        ...
        fields[key] = value;
      ...
      else if(regexps['without-props'].test(line)) {
    	  results = line.match(regexps['without-props']);
    	  key = results[1].replace(regexps['key'], '').toLowerCase();
          var itemNumberMatches = results[1].match(regexps['key']);
          var itemNumber = null;
          
          if(itemNumberMatches) {
          	var s = itemNumberMatches[0];
          	s = s.substr(4); //cut "item"
          	s = s.substr(0, s.length - 1); // remove trailing dot
          	var itemNumber = s;
          } 
	      ...
	      var valueObj = {value : value, item: itemNumber};
	      
	      fields[key] = fields[key] || {};
	      fields[key][type] = fields[key][type] || [];
	      fields[key][type] = fields[key][type].concat(valueObj);
      }
